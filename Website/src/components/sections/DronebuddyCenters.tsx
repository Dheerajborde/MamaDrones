...
const droneCenters: DroneCenter[] = [
  ...
  {
  name: "Vijay Shankar Jadhav (Patil)",
  district: "सातारा",
  coordinates: [74.0042, 17.6678], // Approximate coordinates for Atit, Satara
  ...
  {
  name: "Shivam Madhukar Tale",
  district: "बुलढाणा",
  coordinates: [76.5345, 21.0514], // Approximate coordinates for Jalgaon Jamod, Buldhana, MH
  address: "Shiv Kisaan Drone, Jalgaon Jamod, Buldhana, Maharashtra",
  phone: "8308720010",
  locationLink: "https://maps.app.goo.gl/1TnFcpYK3TcM2ifo9?g_st=aw",
  },
  {
  name: "Pavan Vitthal Dahiphale",
  district: "जालना",
  coordinates: [76.4603, 19.4424], // Approximate coordinates for Pimprakhad Bk., Ghansavangi, Jalna
  ...
  name: "Sumit Thakare",
  district: "यवतमाळ",
  coordinates: [77.7786, 20.2996], // Approximate coordinates for Bori (Arab), Taluka Darwha, Yavatmal
...
const DronebuddyCenters: React.FC<DronebuddyCentersProps> = ({ language }) => {
  ...
  useEffect(() => {
    ...
    const initialMap = new mapboxgl.Map({
      ...
      center: [77.5946, 18.975], // Default center (India)
      zoom: 6,
    });
      ...
      droneCenters.forEach((center) => {
        const popup = new mapboxgl.Popup({ offset: 25, closeButton: false }).setHTML(`
          <div style="font-family: Arial, sans-serif; padding: 10px;">
            <h3 style="font-size: 16px; font-weight: bold; margin-bottom: 5px;">${center.name}</h3>
            <p><strong>${translations[language].district}:</strong> ${center.district}</p>
  ...
  const handleLocationChange = (event: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    const location = droneCenters.find((center) => center.district === locationName);
    if (location && map) {
      const [lng, lat] = location.coordinates;
      map.flyTo({ center: [lng, lat], zoom: 12, essential: true });
      setSelectedAddress(`${location.name} - ${location.address} - ${location.phone}`);
    }
  };
      ...
      <div className="w-full md:w-1/2 flex flex-col items-center">
        ...
        >
          ...
          {droneCenters.map((center, index) => (
            <option key={index} value={center.district}>{center.district}</option>
          ))}
        </select>
      </div>
